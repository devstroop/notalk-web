...
function renderGoConditionals(html: string, data: PageData): string {
  ...
  if (!goVars['child']) goVars['child'] = 'flex items-center gap-2.5 ml-4 px-3 py-2 rounded-md text-xs text-gray-400 hover:text-white hover:bg-white/5 transition-colors'
...
function evalPageTemplate(page: string, data: PageData): string {
  ...
  if (page === 'accounts') {
    ...
    let rowsHtml = ''
    if (accounts.length === 0) {
      rowsHtml = `<tr><td colspan="5"><div class="px-5 py-12 text-center"><p class="text-sm text-gray-500">No accounts yet.</p><a href="/accounts" class="mt-3 inline-flex items-center justify-center rounded-md bg-brand-500 px-4 py-2 text-sm font-medium text-gray-900 hover:bg-brand-600">Create account</a></div></td></tr>`
    } else {
      rowsHtml = accounts
        .map(
          (a) => `
    ...
    let recentHtml = ''
    if (accounts.length === 0) {
      recentHtml = `<div class="px-5 py-12 text-center"><p class="text-sm text-gray-500">No accounts yet.</p><a href="/accounts" class="mt-3 inline-flex items-center justify-center rounded-md bg-brand-500 px-4 py-2 text-sm font-medium text-gray-900 hover:bg-brand-600">Create account</a></div>`
    } else {
      recentHtml = accounts
        .map(
          (a) => `
